...
PC算法 使用一个全连接的无向图，然后使用$G^2$描述条件独立性。
...
先验知识是什么呢？ 比如哪些变量没有父母，哪些变量没有孩子。 比如本地服务的网络延迟（TCP_LATENCY）指标就没有孩子，
因为网络延迟不会导致其他指标的变化。可能的原因包括工作负载（workload），配置，依赖方（他们没有父母了）的TCP_LATENCY。
...
![图2-激进法构建出来的因果图.png](inference.png)
...
保守派算法，跟激进派算法的做法很像，除了以一步做初始化。

我么初始化DAG的方向，在使用PC算法之前。

这个link，TCP_LATENCY和其他指标的方向和
预设根因指标以及其他指标都是之间连接的e。
和激进算法比，保守算法可以捕捉更多的因故宫关系，减少反直觉的因果关系。
举个例子，M1 → SLO的因果关系，
通过激进算法会丢失，
但是保守算法会保留，
同时
...
![图3-保守派构建出来的因果图.png](inference.png)
