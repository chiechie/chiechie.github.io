...
从上述高斯过程可以看出，通过高斯过程给出了f(x)的概率描述，即f(x)在各个x值上的取值概率。那么很自然地，我们希望通过采样来精确这种描述，即选取合适的x点，使得f(x)更加接近目标。

我们看论文的时候经常会在论述acquisition function 的地方看到两种采样思路，探索(exploration)与利用(exploitation),acquisition function的目的就旨在平衡这两种采样过程:
