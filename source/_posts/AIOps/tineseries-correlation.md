...
- 本文将事件（E，Event）和时序（S，metric）相关关系问题转化为两样本问题（two-sample problem），并使用邻近算法（nearest neighbor method）判断是否相关。主要回答了三个问题：
    - E和S之间是否存在相关关系？ 使用邻近算法判断相关性是否存在
